...
const SYSTEM_PROMPT = `你是自戀型人格疾患（Narcissistic Personality Disorder, NPD）領域的資深研究員與科學傳播者。你的任務是：
...
async function callGLM(apiKey, papersData) {
  ...
  const prompt = `以下是 ${REPORT_DATE} 從 PubMed 抓取的最新自戀型人格疾患（NPD）相關文獻（共 ${papersData.count} 篇）。

請進行以下分析，並以 JSON 格式回傳（不要用 markdown code block）：

{
  "date": "${REPORT_DATE}",
  "market_summary": "1-2句話總結今天文獻的整體趨勢與亮點，聚焦自戀型人格疾患",
  "top_picks": [
    {
      "rank": 1,
      "title_zh": "中文標題",
...
每篇 paper 的 tags 請從以下選擇：自戀型人格疾患、病態自戀、誇大自戀、脆弱自戀、暗黑三角、邊緣型人格、反社會人格、心理治療、精神動力治療、基模治療、心智化治療、創傷與依附、神經影像學、社會認知、同理心、羞恥感、自殺防治、憂鬱症、情緒調節、社交媒體、親密關係暴力、強制控制、職場自戀、領導力、身體意象、飲食疾患、運動心理學、青少年、量表與衡鑑、DSM-5、ICD-11、系統性回顧。
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        try {
          ...
          console.error(`[WARN] JSON parse failed (attempt ${attempt + 1}): ${jsonErr.message}`);
          console.error(`[DEBUG] Response first 300 chars: ${text.slice(0, 300)}`);
...
function generateHtml(analysis) {
  const dateStr = analysis.date || REPORT_DATE;
  ...
  const modelUsed = analysis._model || FALLBACK_MODELS[0];
  ...
  <div class="card-header">
    ...
    <span class="${ucls}">${escapeHtml(util)}實用性</span>
  </div>
  ...
  <div class="card-header-row">
    ...
    <span class="${ucls} utility-sm">${escapeHtml(util)}</span>
  </div>
  <h3>${escapeHtml(p.title_zh || p.title_en || "")}</h3>
...
<title>NPD Research Daily &middot; 自戀型人格疾患文獻日報 &middot; ${dateDisplay}</title>
<meta name="description" content="${dateDisplay} 自戀型人格疾患（NPD）文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .links-banner { margin-top: 40px; display: flex; flex-direction: column; gap: 12px; animation: fadeUp 0.5s ease 0.4s both; }
  .link-card { display: flex; align-items: center; gap: 14px; padding: 16px 22px; background: var(--card-bg); border: 1px solid var(--line); border-radius: 24px; text-decoration: none; color: var(--text); transition: all 0.2s; box-shadow: 0 8px 30px rgba(61,36,15,0.04); }
  ...
  @media (max-width: 600px) { .container { padding: 36px 18px 60px; } .summary-card, .news-card { padding: 20px 18px; } .pico-grid { grid-template-columns: 1fr; } footer { flex-direction: column; gap: 6px; text-align: center; } .topic-name { width: 70px; font-size: 11px; } .links-banner { gap: 8px; } }
</style>
</head>
<body>
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>NPD Research Daily &middot; 自戀型人格疾患文獻日報</h1>
  ...
  <footer>
    ...
    <span><a href="https://github.com/u8901006/narcissistic-personality">GitHub</a></span>
  </footer>
</div>
</body>
</html>`;
...
async function main() {
  ...
  if (!papersData?.papers?.length) {
    ...
    const analysis = {
      ...
      keywords: [],
      topic_distribution: {},
      _model: FALLBACK_MODELS[0],
    };
    const html = generateHtml(analysis);
    mkdirSync(dirname(resolve(args.output)), { recursive: true });
    writeFileSync(args.output, html, "utf8");
    console.error(`[INFO] Empty report saved to ${args.output}`);
    return;
  }

  const analysis = await callGLM(apiKey, papersData);
  if (!analysis) {
    console.error("[ERROR] Analysis failed, cannot generate report");
    process.exit(1);
  }

  analysis.date = analysis.date || REPORT_DATE;
  ...
  const summarizedPath = "data/summarized_pmids.json";
  let existing = { pmids: [], lastUpdated: REPORT_DATE };
  try {
    if (existsSync(summarizedPath)) {
      existing = JSON.parse(readFileSync(summarizedPath, "utf8"));
    }
  } catch { /* empty */ }
  const merged = [...new Set([...(existing.pmids || []), ...pmids])];
  writeFileSync(summarizedPath, JSON.stringify({ pmids: merged, lastUpdated: REPORT_DATE }, null, 2), "utf8");
  console.error(`[INFO] Updated summarized PMIDs: ${merged.length} total`);
}
